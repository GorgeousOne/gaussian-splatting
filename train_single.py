...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, voxel_path, voxel_iterations):
    ...
    normal_cam_name = "DSC00087_DxO_83.jpg"
    normal_cam = next(cam for cam in viewpoint_stack if cam.image_name == normal_cam_name)
    normal_cam_idx = viewpoint_stack.index(normal_cam)
    normal_out_dir = os.path.join(scene.model_path, "normals")
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # if normals_l1_weight(iteration) > 0:
        if True:
            normal_learning_rate = .02
            ...
            Ll1normal_pure = (1 - (normals * gt_normals).sum(dim=-1)) .mean()
            # Ll1normal = normals_l1_weight(iteration) - Ll1normal_pure
            Ll1normal = Ll1normal_pure * normal_learning_rate
            loss = Ll1normal
        ...
        # <===
        print("hello", Ll1normal.item())
        tr.show_images_side_by_side(normals, image, None)

        # out_img = np.transpose(image.detach().cpu().numpy(), (1, 2, 0))
        # out_img = cv2.normalize(out_img, None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8U)
        # out_img = out_img[:, :, [2, 1, 0]] # RGB -> BGR conversion
        # cv2.imwrite(os.path.join(normal_out_dir, f"iter_{iteration}.jpg"), out_img)
...
if __name__ == "__main__":
    ...
    # <===
    if not args.start_checkpoint and os.path.exists(args.model_path + "/point_cloud"):
